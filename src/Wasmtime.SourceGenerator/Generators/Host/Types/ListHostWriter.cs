...
public class ListHostWriter(WitType ElementType) : TypeHostWriter(WitTypeKind.List)
{
    ...
    public override void WriteResultGetterInitializer(IndentedStringBuilder sb, string paramName, int index, ITypeContainerResolver resolver)
    {
        ...
        if (blittable != null)
        {
            WriteToArrayBlittable(sb, resolver, parameterName, builderName, blittable);
            return;
        }
        ...
        WriteToArray(sb, resolver, parameterName, builderName, indexName, usePooledSpan: IsBlittablePrimitive, usePooledArray: IsPoolableReferenceType);
    }
    ...
    public override void WriteResultCleanup(IndentedStringBuilder sb, string paramName, int index, ITypeContainerResolver resolver)
    {
        ...
        if (blittable != null)
        {
            WritePooledReturnPrimitive(sb, parameterName, blittable.PrimitiveKind);
        }
        else if (IsBlittablePrimitive)
        {
            WritePooledReturn(sb, resolver, parameterName);
        }
        else if (IsPoolableReferenceType)
        {
            WritePooledReturnReference(sb, resolver, parameterName);
        }
    }
